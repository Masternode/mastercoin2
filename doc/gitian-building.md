...
*Setup instructions for a Gitian build of Mastercoin Core using a Debian VM or physical system.*
...
- [Building Mastercoin Core](#building-mastercoin-core)
...
Clone the git repositories for Mastercoin Core and Gitian.
...
in the Mastercoin Core repository under 'Fetch and create inputs' to install sources which require
...
To build Mastercoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Mastercoin Core repository.
...
```bash
    ...
    From https://github.com/masternode/mastercoin
...
and the mastercoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mastercoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/mastercoin
SIGPATH=/some/root/path/mastercoin-detached-sigs

./bin/gbuild --url mastercoin=${BTCPATH},signature=${SIGPATH} ../mastercoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mastercoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mastercoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mastercoin-osx-build.assert
```
...
[mastercoin/gitian.sigs](https://github.com/masternode/gitian.sigs/) repository, or if that's not possible to create a pull
request.
