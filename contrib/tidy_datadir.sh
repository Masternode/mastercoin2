...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Mastercoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Mastercoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Mastercoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Mastercoin 0.7 datadir."
    ;;
  3)
    echo "Detected Mastercoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Mastercoin 0.8 datadir."
    ;;
esac
